...
date=`date +%Y%m%d`
outputname="$romtypename-$outputtype-$sourcever-$date-ShadesOfDeath"
...
sed -i "s/$bdisplay/$displayid2=Ported\.by\.ShadesOfDeath\.using\.ErfanTools/" $systemdir/system/build.prop
